...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        new_knowledge = Sentence(newCells, count)
        ...
        for sentenceA, sentenceB in itertools.combinations(self.knowledge, 2):
            if sentenceA != sentenceB and len(sentenceA.cells) > 0 and len(
                    sentenceB.cells) > 0 and sentenceA.cells.issubset(
                    sentenceB.cells):
                newSet = sentenceB.cells - sentenceA.cells
                newCount = sentenceB.count - sentenceA.count
                if len(newSet) > 0 and newCount >= 0:
                    more_sentences.append(Sentence(newSet, newCount))

        for newSentence in more_sentences:
